...
if [ "$#" != 1 ];then
   ...
   echo "# invoke the shellscript by providing command line arguments e.g. ./stopContainers.sh space"
   exit 
fi
blmxspace="$1"
...
if [ $logerr -eq 1 ]; then
 ...
 echo "# Following Containers are about to stop "
 echo "#######################################################################"
 echo $containers
 echo "#######################################################################"
 echo "Triggering stop commands for the containers"
